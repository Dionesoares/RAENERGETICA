...
import { Plus, TrendingUp, TrendingDown, Wallet, FileDown, Sheet, Trash2, Pencil } from "lucide-react";
...
export default function AdminFinancial() {
  ...
  const handleSave = async (form) => {
    if (editing) await base44.entities.Transaction.update(editing.id, form);
    else await base44.entities.Transaction.create(form);
    setModalOpen(false);
    setEditing(null);
    load();
  };
  ...
  const totals = useMemo(() => {
    const income = transactions.filter((t) => t.type === "entrada").reduce((s, t) => s + Number(t.amount || 0), 0);
    const expense = transactions.filter((t) => t.type === "saida").reduce((s, t) => s + Number(t.amount || 0), 0);
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {transactions.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-muted-foreground">Nenhum lançamento.</td></tr>
              )}
              {transactions.map((t) => (
                <tr key={t.id}>
